...
XREF = np.vstack((np.sin(T), np.cos(T), np.zeros_like(T)))
...
V_MIN = -0.1
V_MAX = 0.1   # Right wheel angular velocity limits
OMEGA_MIN = -0.5
OMEGA_MAX = 0.5   # Left wheel angular velocity limits
XMIN = -1 * 0.2 *  np.array([1.0 ,1.0, 1.0])#np.ones(3)          # State limits
XMAX =  1 * 0.2 * np.array([1.0 ,1.0, 1.0]) #np.ones(3) 

# Weights for cost function
Q = Q = np.diag([Q_VAL, Q_VAL, 1])      
R = R_VAL * np.eye(DU)
P = P_VAL * Q
...
class ros_ref_tracking(Node):
    def __init__(self):
        ...
        self.ax.set_ylim(-2, 2)  # Limite initiale de l'axe y
