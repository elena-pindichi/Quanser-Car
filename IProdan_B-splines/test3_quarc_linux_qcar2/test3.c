 ...
 * C source code generated on : Mon May  5 15:49:17 2025
 *
...
void test3_output(void)
{
  ...
  for (nm1d2 = 0; nm1d2 <= ntilerows; nm1d2++) {
    ...
    horizon_idx->data[nm1d2] = varargout_1 + 1.0;
  }
  ...
  test3_emxInit_real_T(&wref, 2);

  /* MATLAB Function: '<Root>/MATLAB Function' */
  ibcol = wref->size[0] * wref->size[1];
  wref->size[0] = 2;
  wref->size[1] = horizon_idx->size[1];
  test3_emxEnsureCapacity_real_T(wref, ibcol);
  for (nm1d2 = 0; nm1d2 < ntilerows; nm1d2++) {
    ibcol = nm1d2 << 1;
    ub->data[ibcol] = 0.4;
    ub->data[ibcol + 1] = 0.2;
    ...
    wref->data[nm1d2 << 1] = d[ibcol];
    wref->data[1 + (nm1d2 << 1)] = d[ibcol + 1];
  }
   ...
   */
  test3_fmincon(expl_temp.x0, c, wref, (real_T)horizon_idx->size[1], tmp, lb, ub,
                expl_temp, u_opt);
  test3_emxFree_real_T(&c);
  ...
  /* '<S3>:1:43' */
  /* '<S3>:1:47' */
  Vr_data = rt_atan2d_snf(rtb_FromWorkspace[3] / ndbl, rtb_FromWorkspace[2] /
    ndbl);

  /* '<S3>:1:48' */
  phir_data = rtb_FromWorkspace[2] * rtb_FromWorkspace[5] - rtb_FromWorkspace[3]
    * rtb_FromWorkspace[4];
  apnd = phir_data;
  phir_data = 0.256 * phir_data / rt_powd_snf(ndbl, 3.0);

  /* '<S3>:1:50' */
  uref_data[0] = ndbl;
  uref_data[1] = ((rtb_FromWorkspace[2] * rtb_FromWorkspace[7] -
                   rtb_FromWorkspace[3] * rtb_FromWorkspace[6]) * ndbl * ndbl -
                  (rtb_FromWorkspace[2] * rtb_FromWorkspace[4] +
                   rtb_FromWorkspace[3] * rtb_FromWorkspace[5]) * (3.0 * apnd)) *
    (0.256 * ndbl) / (apnd * apnd * 0.065536 + rt_powd_snf(ndbl, 6.0));
  apnd = Vr_data;
  ndbl = cos(Vr_data);
  Vr_data = ndbl;
  cdiff = atan(phir_data);
  phir_data = apnd + cdiff;
  b_data = phir_data;
  b_b_data = apnd;
  apnd = sin(phir_data);
  phir_data = apnd;
  b_b_data = sin(b_b_data);
  b_data = cos(b_data);
  ...
  /* '<S3>:1:66' */
  /* '<S3>:1:68' */
  varargout_1 = tan(cdiff);
  cdiff = uref_data[0];
  x_idx_1 = uref_data[1];
  wref->data[0] = (ndbl - (0.35 * apnd / 0.256 + b_b_data) * varargout_1) *
    cdiff + -0.35 * apnd * x_idx_1;
  wref->data[1] = ((0.35 * b_data / 0.256 + ndbl) * varargout_1 + b_b_data) *
    cdiff + 0.35 * b_data * x_idx_1;

  /* '<S3>:1:71' */
  /* '<S3>:1:72' */
  /* '<S3>:1:86' */
  varargout_1 = fmin(rtb_Clock + 5.0, 1.0);
  if (didZcEventOccur) {
    ibcol = horizon_idx->size[0] * horizon_idx->size[1];
    horizon_idx->size[0] = 1;
    horizon_idx->size[1] = 1;
    test3_emxEnsureCapacity_real_T(horizon_idx, ibcol);
    horizon_idx->data[0] = (rtNaN);
  } else if (varargout_1 < rtb_Clock) {
    horizon_idx->size[0] = 1;
    horizon_idx->size[1] = 0;
  } else if ((rtIsInf(rtb_Clock) || rtIsInf(varargout_1)) && (rtb_Clock ==
              varargout_1)) {
    ibcol = horizon_idx->size[0] * horizon_idx->size[1];
    horizon_idx->size[0] = 1;
    horizon_idx->size[1] = 1;
    ...
    ibcol = horizon_idx->size[0] * horizon_idx->size[1];
    horizon_idx->size[0] = 1;
    ntilerows = (int32_T)(varargout_1 - rtb_Clock);
    horizon_idx->size[1] = ntilerows + 1;
    test3_emxEnsureCapacity_real_T(horizon_idx, ibcol);
    for (nm1d2 = 0; nm1d2 <= ntilerows; nm1d2++) {
      horizon_idx->data[nm1d2] = rtb_Clock + (real_T)nm1d2;
    ...
    ndbl = floor((varargout_1 - rtb_Clock) + 0.5);
    apnd = rtb_Clock + ndbl;
    cdiff = apnd - varargout_1;
    if (fabs(cdiff) < 4.4408920985006262E-16 * fmax(fabs(rtb_Clock), fabs
         (varargout_1))) {
      ndbl++;
      apnd = varargout_1;
    } else if (cdiff > 0.0) {
  ...
  uref_data[0] = (0.256 * Vr_data + rtb_FromWorkspace[0]) + 0.35 * b_data;
  uref_data[1] = (0.256 * b_b_data + rtb_FromWorkspace[1]) + 0.35 * phir_data;
  k = horizon_idx->size[1] << 1;
  test3_emxInit_real_T(&tmp_0, 2);

  /* MATLAB Function: '<Root>/MATLAB Function2' */
  ibcol = tmp_0->size[0] * tmp_0->size[1];
  tmp_0->size[0] = 2;
  tmp_0->size[1] = horizon_idx->size[1];
  test3_emxEnsureCapacity_real_T(tmp_0, ibcol);
  ntilerows = horizon_idx->size[1];
  test3_emxInit_real_T(&wref_0, 2);

  /* MATLAB Function: '<Root>/MATLAB Function2' */
  ibcol = wref_0->size[0] * wref_0->size[1];
  wref_0->size[0] = 2;
  wref_0->size[1] = horizon_idx->size[1];
  test3_emxEnsureCapacity_real_T(wref_0, ibcol);
  for (nm1d2 = 0; nm1d2 < ntilerows; nm1d2++) {
    ndbl = horizon_idx->data[nm1d2];
    ibcol = ((int32_T)ndbl - 1) << 1;
    Vr_data_tmp = nm1d2 << 1;
    tmp_0->data[Vr_data_tmp] = uref_data[ibcol];
    uref_data_tmp = (nm1d2 << 1) + 1;
    tmp_0->data[uref_data_tmp] = uref_data[1 + ibcol];
    wref_0->data[Vr_data_tmp] = wref->data[ibcol];
    wref_0->data[uref_data_tmp] = wref->data[1 + ibcol];
  }

  test3_emxFree_real_T(&wref);
  ...
  test3_fmincon_l(expl_temp.x0, tmp_0, wref_0, (real_T)horizon_idx->size[1], tmp,
                  expl_temp_0, lb);
  ...
  /* '<S3>:1:134' */
  varargout_1 = cos(test3_B.Integrator);
  cdiff = tan(test3_X.Integrator3_CSTATE);
  apnd = sin(test3_B.Integrator);
  b_b_data = varargout_1 - (0.35 * rtb_Clock / 0.256 + apnd) * cdiff;
  b_data = -0.35 * rtb_Clock;
  x_idx_1 = (0.35 * ndbl / 0.256 + varargout_1) * cdiff + apnd;
  cdiff = 0.35 * ndbl;

  /* '<S3>:1:120' */
  if (fabs(x_idx_1) > fabs(b_b_data)) {
    rtb_Clock = b_b_data / x_idx_1;
    ndbl = 1.0 / (rtb_Clock * cdiff - b_data);
    Vr_data = cdiff / x_idx_1 * ndbl;
    cdiff = -ndbl;
    b_b_data = -b_data / x_idx_1 * ndbl;
    ndbl *= rtb_Clock;
  } else {
    rtb_Clock = x_idx_1 / b_b_data;
    ndbl = 1.0 / (cdiff - rtb_Clock * b_data);
    Vr_data = cdiff / b_b_data * ndbl;
    cdiff = -rtb_Clock * ndbl;
    b_b_data = -b_data / b_b_data * ndbl;
  }
  ...
  if (test3_P.UpNLDownFL2_CurrentSetting == 1) {
    b_b_data = u_opt->data[0];
  } else {
    b_b_data = Vr_data * lb->data[0] + b_b_data * lb->data[1];
  }

  /* Saturate: '<S9>/command saturation' incorporates:
   *  ManualSwitch: '<Root>/Up - NL, Down - FL 2'
   */
  if (b_b_data > test3_P.commandsaturation_UpperSat) {
    b_b_data = test3_P.commandsaturation_UpperSat;
  } else if (b_b_data < test3_P.commandsaturation_LowerSat) {
    b_b_data = test3_P.commandsaturation_LowerSat;
  }

  /* Product: '<S9>/Multiply1' incorporates:
   *  Saturate: '<S9>/command saturation'
   */
  test3_B.desired = b_b_data * test3_B.switch_l;
   ...
   *  Sum: '<S9>/Add'
   */
  b_b_data = (test3_P.Kpms_Gain * rtb_Clock + test3_X.Integrator1_CSTATE_ng) +
    test3_P.Kffms_Gain * test3_B.desired;

  /* Saturate: '<S9>/command saturation1' */
  if (b_b_data > test3_P.commandsaturation1_UpperSat) {
    b_b_data = test3_P.commandsaturation1_UpperSat;
  } else if (b_b_data < test3_P.commandsaturation1_LowerSat) {
    b_b_data = test3_P.commandsaturation1_LowerSat;
  }
   ...
   *  Saturate: '<S9>/command saturation1'
   */
  b_b_data = b_b_data * test3_B.switch_l * test3_P.directionconvention_Gain;

  /* Saturate: '<S8>/command saturation' */
  if (b_b_data > test3_P.commandsaturation_UpperSat_n) {
    /* Saturate: '<S8>/command saturation' */
    test3_B.commandsaturation = test3_P.commandsaturation_UpperSat_n;
  } else if (b_b_data < test3_P.commandsaturation_LowerSat_i) {
    /* Saturate: '<S8>/command saturation' */
    test3_B.commandsaturation = test3_P.commandsaturation_LowerSat_i;
  } else {
    /* Saturate: '<S8>/command saturation' */
    test3_B.commandsaturation = b_b_data;
  }
  ...
  /* Product: '<S4>/Product' */
  test3_B.Product_f = varargout_1 * test3_B.Gain3;
...
RT_MODEL_test3_T *test3(void)
{
  ...
  /* External mode info */
  test3_M->Sizes.checksums[0] = (4057457759U);
  test3_M->Sizes.checksums[1] = (2523954435U);
  test3_M->Sizes.checksums[2] = (2104935243U);
  test3_M->Sizes.checksums[3] = (984682817U);
