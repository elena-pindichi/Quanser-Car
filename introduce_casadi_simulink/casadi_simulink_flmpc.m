...
% Delta = 0.35;
% Q_val = 30;  R_val = 1;
% rhat  = min(Delta*l*10/sqrt(Delta^2 + l^2),1);
% 
% Q = Q_val*eye(2);
% R = R_val*eye(2);
% A = eye(2);
...
%          xref(2)+l*sin(xref(3))+Delta*sin(xref(3)+xref(4)) ];
% 
% %%— build NLP over Z and W only —%%
% w    = {}; w0    = []; lbw = []; ubw = [];
% g    = {}; lbg   = []; ubg = [];
% J    = 0;
% 
% % initial flat output Z0
% Zk = MX.sym('Z0',2);
% w   = {w{:}, Zk};
% w0  = [w0; zref];        % init guess at reference
% lbw = [lbw; -inf; -inf];
% ubw = [ubw;  inf;  inf];
% 
% for k = 0:N-1
%   % virtual input
%   Wk = MX.sym(['W_' num2str(k)],2);
%   w   = {w{:}, Wk};
%   w0  = [w0; 0; 0];
%   lbw = [lbw; -inf; -inf];
%   ubw = [ubw;  inf;  inf];
% 
%   % next flat output
%   Zk_next = MX.sym(['Z_' num2str(k+1)],2);
%   w   = {w{:}, Zk_next};
%   w0  = [w0; zref];       % warm start
%   lbw = [lbw; -inf; -inf];
%   ubw = [ubw;  inf;  inf];
% 
%   % stage cost
%   J = J + (Zk-zref)'*Q*(Zk-zref) + Wk'*R*Wk;
% 
%   % linear flat-output dynamics
%   g   = {g{:}, Zk_next - A*Zk - B*Wk};
%   lbg = [lbg; 0; 0];
%   ubg = [ubg; 0; 0];
% 
%   % norm constraint on Wk
%   g   = {g{:}, Wk'*Wk};
%   lbg = [lbg; -inf];
%   ubg = [ubg;  rhat];
% 
...
% end
% 
% %%— create and solve solver symbolically —%%
% prob   = struct('f',J, 'x',vertcat(w{:}),'g',vertcat(g{:}));
% opts   = struct('ipopt',struct('print_level',0),'print_time',false);
% solver = nlpsol('solver','ipopt',prob,opts);
% 
% %%— export function Z0→W0 —%%
% Z0 = MX.sym('Z0',2);
% % lower/upper bounds: clamp only the first two unknowns (Z0) to the input Z0
% lbw_sym = MX(lbw); ubw_sym = MX(ubw);
% lbw_sym(1:2) = Z0; 
% ubw_sym(1:2) = Z0;
% 
% sol = solver('x0',w0,'lbx',lbw_sym,'ubx',ubw_sym,'lbg',lbg,'ubg',ubg);
% % extract W0 (decision vars are ordered [Z0; W0; Z1; W1; Z2; …])
% W0 = sol.x(3:4);
% 
% mpc_fun = Function('mpc_fun',{Z0},{W0});
% mpc_fun.save('mpc_fun.casadi');
% 
% disp('Saved mpc_fun: Z0 → W0');
