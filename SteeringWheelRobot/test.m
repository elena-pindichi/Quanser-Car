...
%% Define Obstacles using MPT3 Polyhedron
obstacles = {
    Polyhedron([2 2; 3 2; 3 3; 2 3]),  % Square obstacle
    Polyhedron([5 1; 6 2; 5.5 3; 4.5 2]),  % Quadrilateral
    Polyhedron([7 4; 8 5; 7.5 6; 6.5 5])   % Another polygon
};
...
for i = 1:length(obstacles)
    obstacles{i}.plot('color', 'r', 'alpha', 0.3);
end
...
n = 15;
d=5; 
knot=[0 10]; % from 1 to 10 second
knot = [0 50]; % Extends trajectory duration from 10s to 50s
...
wpknot=linspace(min(knot),max(knot)-0.0001,size(W,2));
...
function is_inside = check_polyhedron(P, obstacles)
    is_inside = false;
    for obs = 1:length(obstacles)
        if obstacles{obs}.contains(P')
            is_inside = true;
            return;
        end
    end
