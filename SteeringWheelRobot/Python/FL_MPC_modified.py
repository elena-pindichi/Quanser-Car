...
ZREF_PARAM = solver.parameter(DZ, NPRED)
WREF_PARAM = solver.parameter(DU, NPRED)
...
objective = 0
for k in range(0, NPRED-1):
    objective += casadi.mtimes([(Z[:, k] - ZREF_PARAM[:, k]).T, Q, (Z[:, k] - ZREF_PARAM[:, k])]) + \
                    casadi.mtimes([(W[:, k] - WREF_PARAM[:, k]).T, R, (W[:, k] - WREF_PARAM[:, k])]) 
k=NPRED-1
objective += casadi.mtimes([(Z[:, k] - ZREF_PARAM[:, k]).T, P, (Z[:, k] - ZREF_PARAM[:, k])])
