...
%% Parameters
N_pred_val = 10;        % Prediction horizon
...
x0 = [0.3; 0.2; -pi/2; pi/3];  % Initial state [x; y; theta; phi]
u0 = zeros(du, 1);
...
for i = 1 : Nsim
    ...
    for k = 1 : Npred                                                      
        objective = objective + (x(:, k) - xref(:, i + k - 1))' * Q * (x(:, k) - xref(:, i + k - 1)) + ...
                                 (u(:, k) - uref(:, i + k - 1))' * R * (u(:, k) - uref(:, i + k - 1));
    end

    objective = objective + (x(:, Npred + 1) - xref(:, i + Npred))' * P * (x(:, Npred + 1) - xref(:, i + Npred));
