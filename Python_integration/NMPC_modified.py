...
XREF_PARAM = solver.parameter(DX, NPRED)
UREF_PARAM = solver.parameter(DU, NPRED)
...
objective = 0
for k in range(0, NPRED-1):
    objective += casadi.mtimes([(X[:, k] - XREF_PARAM[:, k]).T, Q, (X[:, k] - XREF_PARAM[:, k])]) + \
                    casadi.mtimes([(U[:, k] - UREF_PARAM[:, k]).T, R, (U[:, k] - UREF_PARAM[:, k])])
k=NPRED-1
objective += casadi.mtimes([(X[:, k] - XREF_PARAM[:, k]).T, P, (X[:, k] - XREF_PARAM[:, k])])
