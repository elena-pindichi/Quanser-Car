...
def create_ellipse(mat_A, c, num_points=100):
    angles = np.linspace(0, 2 * np.pi, num_points)
    circle = np.array([np.cos(angles), np.sin(angles)])
    ellipse = np.linalg.inv(mat_A) @ circle + c[:, None]
    return ellipse[0, :], ellipse[1, :]

# Given vertices
V = np.array([[2.5, 1.2], [2, 0.5], [1.4, 1]])
Pv = Polygon(V)  # Replace with PyPolyhedron or similar library to handle Polyhedron
mat_A, c = mvee(V, tol=0.0001)
x_ellipse, y_ellipse = create_ellipse(mat_A, c)

# Enlarged ellipse
eigvals, eigvecs = np.linalg.eig(mat_A)
lambda_enlarged = 0.9 * eigvals
mat_A_enlarged = eigvecs @ np.diag(lambda_enlarged) @ np.linalg.inv(eigvecs)
x_ellipse_enlarged, y_ellipse_enlarged = create_ellipse(mat_A_enlarged, c)
...
#  Initial condition
X0 = np.array([-2, 0.2, 0])  # Initial state [x; y; theta]
XREF = np.array([4, 1, 0])    # Reference state [x_ref; y_ref; theta_ref]
...
Q = np.vstack([[100, 0, 0], [0, 100, 0], [0, 0, 0.00001]])#Q_VAL * np.eye(DX)
R = R_VAL * np.eye(DU)
P = P_VAL * Q
...
#  Nonlinear dynamics of the unicycle model with omega_r and omega_l
f_dynamics = cas.vertcat(u[0] * cas.cos(x[2]), u[0] * cas.sin(x[2]), u[1])
...
for k in range(0, NPRED) : 
    ...
    solver.subject_to((x[:2, k] - c).T @ mat_A @ (x[:2, k] - c) > 1 + 0.1)
...
plt.figure()
plt.scatter(xsim[0, :], xsim[1, :], label="State Trajectory")
# plt.plot(Pv)
plt.plot(x_ellipse, y_ellipse, label="Original Ellipse")
plt.plot(x_ellipse_enlarged, y_ellipse_enlarged, label="Enlarged Ellipse")
plt.scatter(c[0], c[1], label="Center", color='red')
